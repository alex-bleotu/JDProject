...
export const RecyclingProvider = ({ children }: { children: ReactNode }) => {
    ...
    const fetchRecyclingData = async (userAddress: string) => {
        try {
            ...
            const fetchPlastic = fetch(
                "http://95.179.246.157:3005/api/get/plastic",
                {
                    method: "POST",
                    headers: {
            ...
            const fetchGlass = fetch(
                "http://95.179.246.157:3005/api/get/glass",
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
            ...
            const fetchMetal = fetch(
                "http://95.179.246.157:3005/api/get/metal",
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
            ...
            const [plasticResponse, glassResponse, metalResponse] =
                await Promise.all([fetchPlastic, fetchGlass, fetchMetal]);
